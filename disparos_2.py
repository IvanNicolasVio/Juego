...
class lista_balas_enemigas():
    def __init__(self,cantidad) -> None:
        self.cantidad_balas = cantidad
        #self.lista_balas_en_cargador = self.crear_balas(enemigo.lista_balas_en_cargador)
        self.lista_balas_usadas = []
        self.bandera_z = False
    ...
    def crear_balas(self,lista_de_enemigos):
        for enemigo in lista_de_enemigos:
            enemigo.cargador_balas.append(Bala(0,0,5))
    ...
    def update(self,enemigo,screen,jugador,lista_de_enemigos,delta_ms):
        self.dibujar_balas(screen)
        self.disparar_balas(lista_de_enemigos,delta_ms)
        # for enemigos in lista_de_enemigos:
        #     if not enemigos.cargador_balas:
        #         self.crear_balas()
...
#-----------------------------------------------------------------------------------------------------------------------DISPAROS ENEMIGOS
    ...
    def disparar_balas(self,lista_de_enemigos,delta_ms):
        for enemigo in lista_de_enemigos:
            self.tiempo_acumulado += delta_ms
            if self.tiempo_acumulado > 1800:
                if enemigo.bandera_para_disparar and enemigo.cargador_balas and enemigo.poder_disparar:
                    self.tiempo_acumulado = 0
                    bala_disparada = enemigo.cargador_balas.pop(0)
