...
class Button_v2(Widget3):
    def __init__(self,master,x=0,y=0,w=200,h=50,color_background=BLUE,color_border=BLUE,image_background=None,text="Button",font="Arial",font_size=14,font_color=BLACK,on_click=None,on_click_param=None):
    ...
    def render(self):
        super().render()
        if not self.bandera_para_bloquear:
            if self.state == M_STATE_HOVER: # Se aclara la imagen
                self.slave_surface.fill(M_BRIGHT_HOVER, special_flags=pygame.BLEND_RGB_ADD) 
            elif self.state == M_STATE_CLICK: # Se oscurece la imagen
                self.slave_surface.fill(M_BRIGHT_CLICK, special_flags=pygame.BLEND_RGB_SUB) 
